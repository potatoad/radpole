...
func _on_Power_up_spawn_timer_timeout():
	var power_ups_number = round(rand_range(0, power_ups.size() - 1))
	Global.instance_node(power_ups[power_ups_number], Vector2(rand_range(0, 640), rand_range(0, 360)), self)
